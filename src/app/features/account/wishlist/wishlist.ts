...
export class AccountWishlist {
    ...
    addToCart(productId: number) {
        this.cartService.addToCart(productId, 1).pipe(first()).subscribe({
    ...
    addAllToCart() {
        const validItems = this.wishlistItems().filter(
            (item) => !item.isDeleted && item.inStock
        );
        ...
        // 4. Call API
        this.cartService.updateCart(itemsToUpdate).pipe(first()).subscribe({
            next: () => {
                this.toastr.success(`${addedCount} items added to cart`);
            },
            error: (err) => {
                this.toastr.error(err.error || 'Failed to update cart');
            }
        });
    }
}
